...
A sandbox that **stands in for the cybersecurity tools a client runs** — VirusTotal, CrowdStrike, QRadar, AbuseIPDB, Zscaler, Forcepoint DLP and ~50 more — so our agents can be exercised end‑to‑end against realistic, logged mock APIs instead of touching production systems (or paying per call).
...
- **53 emulated tools** across 17 categories (threat‑intel, EDR, SIEM, DLP, identity, AI‑guardrails, vuln‑management, SOAR, …), each at a stable base path with realistic endpoints.
- **8 high‑fidelity tools** with hand‑authored, input‑seeded responses: **VirusTotal, AbuseIPDB, CrowdStrike Falcon, 2Captcha, IBM QRadar, Qualys, Hybrid Analysis, Nightfall.ai**. The same indicator (hash / IP / domain) always yields the same verdict, like a real reputation service.
- **45 scaffolded tools** with representative endpoints and templated responses (fresh ids/timestamps each call).
...
# AbuseIPDB — IP abuse score (auth: Key header)
curl "http://localhost:3002/api/mock/abuseipdb/api/v2/check?ipAddress=118.25.6.39&maxAgeInDays=90" \
  -H "Key: $KEY"
...
# Nightfall — scan text for PII/secrets (auth: Bearer)
curl -X POST "http://localhost:3002/api/mock/nightfall/v3/scan" \
  -H "Authorization: Bearer $KEY" -H "content-type: application/json" \
  -d '{"payload":["My SSN is 123-45-6789 and card 4111 1111 1111 1111"]}'

# 2Captcha — no auth
curl -X POST "http://localhost:3002/api/mock/2captcha/createTask" \
  -H "content-type: application/json" \
  -d '{"task":{"type":"TurnstileTaskProxyless","websiteURL":"https://x.com","websiteKey":"0x4AAA"}}'
```
...
| **Tool Catalog** | Searchable/filterable grid of all 53 tools |
...
lib/
  tools/registry.ts             the catalog (source of truth)
  tools/crafted/*               8 high‑fidelity tools
  tools/generic.ts              45 scaffolded tools
