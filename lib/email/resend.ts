...
function resetHtml({ name, resetUrl }: ResetEmailInput): string {
  const greeting = name ? `Hi ${escapeHtml(name)},` : "Hi,";
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f2f2f5;padding:32px 16px;">
    <tr><td align="center">
      <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="max-width:480px;background:#ffffff;border:1px solid #e4e4ea;">
        ...
        <tr><td style="padding:16px 28px 4px;font-size:14px;line-height:1.5;">
          <p style="margin:0 0 14px;">${greeting}</p>
          <p style="margin:0 0 14px;">We received a request to reset the password for your <strong>Client Tool Emulator</strong> account. Choose a new password to continue.</p>
        </td></tr>
        <tr><td style="padding:8px 28px 20px;">
          <a href="${url}" style="display:inline-block;background:${ACCENT};color:${INK};text-decoration:none;font-weight:bold;font-size:14px;padding:11px 22px;">Reset your password</a>
        </td></tr>
        <tr><td style="padding:0 28px 24px;font-size:12px;line-height:1.5;color:#6b6b78;">
          <p style="margin:0 0 8px;">This link expires in 1 hour and can be used once. If the button doesn't work, paste this URL into your browser:</p>
          <p style="margin:0;word-break:break-all;"><a href="${url}" style="color:#8a7400;">${url}</a></p>
          <p style="margin:14px 0 0;">If you didn't request this, you can safely ignore this email — your password stays unchanged.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
function resetText({ name, resetUrl }: ResetEmailInput): string {
  const greeting = name ? `Hi ${name},` : "Hi,";
  return `${greeting}

We received a request to reset the password for your Client Tool Emulator account. Choose a new password here:
...
export async function sendPasswordResetEmail(input: ResetEmailInput): Promise<SendResult> {
  ...
  try {
    const res = await fetch(RESEND_ENDPOINT, {
      ...
      body: JSON.stringify({
        from,
        to: [input.to],
        subject: "Reset your Client Tool Emulator password",
        html: resetHtml(input),
